...
export function TestimonialsSection(){return <section className="grid gap-4 md:grid-cols-2">{reviews.map((review,index)=><Card key={review.name} className="overflow-hidden"><img src={portraits[index]} alt={review.name} className="h-44 w-full object-cover sm:h-52" loading="lazy" /><CardContent className="pt-6 text-sm"><Quote className="mb-3 size-5 text-primary" /><p className="text-base">"{review.quote}"</p><p className="mt-3 font-medium">{review.name}</p><p className="text-muted-foreground">{review.role}</p></CardContent></Card>)}</section>;}
